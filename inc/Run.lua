...
syria = Token:match("(%d+)")
redis:set(syria..":VERSION",2.9)
redis:set(syria..":SUDO_ID:",SUDO_USER)
redis:set(syria..":DataCenter:",'German')
redis:set(syria..":UserNameBot:",BOT_User)
redis:set(syria..":NameBot:",BOT_NAME)
redis:hset(syria..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3syria_INSTALL","Yes")
info = {}
...
syria = Token:match("(%d+)")
our_id = tonumber(syria)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(syria..":UserNameBot:")
SUDO_ID = tonumber(redis:get(syria..":SUDO_ID:"))
SUDO_USER = redis:hgetall(syria..'username:'..SUDO_ID).username
version = redis:get(syria..":VERSION")
DataCenter = redis:get(syria..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..syria..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(syria..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(syria..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(syria..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(syria..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(syria..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي👷🏽'
msg.TheRank = 'المنشىء الاساسي👷🏽 '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(syria..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(syria..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(syria..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(syria..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(syria..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(syria..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(syria..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(syria..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(syria..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(syria..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(syria..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(syria..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(syria..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(syria..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.syria then 
if msg.text and ScriptFile.isyria then
for k, syria in pairs(ScriptFile.syria) do
local SearchText = msg.text:match(syria)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.isyria(msg,{msg.text:match(syria)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",syria.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dsyria then
if ScriptFile.dsyria(msg) == false then
return false
end
...
if Plug.syria then 
if msg.text and Plug.isyria then
for k, syria in pairs(Plug.syria) do
local SearchText = msg.text:match(syria)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(syria..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(syria..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.isyria(msg,{msg.text:match(syria)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",syria.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source syria")
end 

end
else
print("The File Script.lua Not Runing in The Source syria")
...
function tdcli_update_callback(data)
	...
	redis:srem(syria..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/syriaTEAM/SY/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/syriaTEAM/SY/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/syriaTEAM/SY/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/syriaTEAM/SY/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/syriaTEAM/SY/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/syriaTEAM/SY/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(syria..":SUDO_ID:")
redis:hset(syria..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(syria..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(syria..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(syria..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(syria..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(syria..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(syria..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(syria..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(syria..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(syria..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(syria..'group:add'..data.chat_.id_) then
	redis:set(syria..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(syria..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(syria..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(syria..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	syria..'group:name'..k,IDS.Title,
	syria..'num_msg_syria'..k,5,
	syria..'group:add'..k,true,
	syria..'lock_link'..k,true,
	syria..'lock_spam'..k,true,
	syria..'lock_webpage'..k,true,
	syria..'lock_markdown'..k,true,
	syria..'lock_flood'..k,true,
	syria..'lock_bots'..k,true,
	syria..'mute_forward'..k,true,
	syria..'mute_contact'..k,true,
	syria..'mute_document'..k,true,
	syria..'mute_inline'..k,true,
	syria..'lock_username'..k,true,
	syria..'replay'..k,true
	)
	redis:sadd(syria..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(syria..'username:'..ID,'username',user)
	redis:sadd(syria..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(syria..'username:'..ID,'username',user)
	redis:sadd(syria..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(syria..'username:'..ID,'username',user)
	redis:sadd(syria..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(syria..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(syria..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(syria..'username:'..data.user_.id_).username
	...
	redis:hset(syria..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
